...
from costsearch import Graph_costsearch
...
class Ui_AISearchingTechniquesMainWindow(object):
        ...
        with open('test.txt', 'w') as f:
            ...
            if (graphType == "Undirectd Graph"):
                if searchType == "BFS":
                    ...
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path1, goal = graphdfs.depth_first_search(start, goals)
                    if (traced_path1):
                        print('Path:', end=' ')
                        Graph_dfs.print_path(traced_path1, goal)
                        print()
                elif searchType == "cost_search":
                    graphcost = Graph_costsearch(directed=False)
                    for i in range(0, self.counter):
                        graphcost.add_edge(Node1_arr[i], Node2_arr[i], self.EdgeWeight_arr[i])
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path1, cost = graphcost.cost_search(start, goals)
                    if (traced_path1):
                        print('Path:', end=' ')
                        Graph_costsearch.print_path(traced_path1, cost)
                        print()
                # elif searchType == "A*":
                #     for i in range(0, self.counter):
                ...
                #     start = self.StartNode_input.text()
                #     goals = Goal_list
                #     traced_path5, cost3, goal1 = self.graphastar.a_star_search(start, goals)
                #     if (traced_path5):
                #         print('Path:', end=' ')
                #         Graph_astar.print_path(traced_path5, goal1)
                #         print('\nCost:', cost3)


                elif searchType == "Depth_limited_search":
                    graphcost = Graph_costsearch(directed=False)
                    for i in range(0, self.counter):
                        graphcost.add_edge(Node1_arr[i], Node2_arr[i], self.EdgeWeight_arr[i])
                    start = self.StartNode_input.text()
                ...
                if searchType == "BFS":
                    ...
                    graphcost = Graph_costsearch(directed=False)
                    for i in range(0, self.counter):
                        self.graphcost.add_edge(Node1_arr[i], Node2_arr[i], self.EdgeWeight_arr[i])
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path, goal, cost = graphcost.cost_search(start, goals)
                    if traced_path:
                        print('Path:', end=' ')
                        Graph_costsearch.print_path(traced_path, goal)
                        print('Cost:', cost)
                    else:
                        print('No path found')
    ...
    def retranslateUi(self, AISearchingTechniquesMainWindow):
        ...
        self.SearchTypecomboBox.setItemText(3, _translate("AISearchingTechniquesMainWindow", "cost_search"))
        self.SearchTypecomboBox.setItemText(4, _translate("AISearchingTechniquesMainWindow", "Depth_limited_search"))
